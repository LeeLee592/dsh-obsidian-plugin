...
- **`obsidian_plugin_e2e`**：脚手架化沙箱端到端测试（L4 档）——WebdriverIO + `wdio-obsidian-service` 会先解析出要用的测试库（复用项目里已有的测试库；没有就建一个空的 `e2e/vault`，因为配置指向不存在的库会让每次运行在 onPrepare 直接失败），再启动一个**独立的 Obsidian**（独立配置目录 + 库副本，始终以 `copy: true` 打开，测试只读副本），不切你的窗口、不抢焦点；`init` 写入 WebdriverIO 配置（`wdio.conf.mts`、`tsconfig.e2e.json` 与起始 spec——脚手架目录由 `dir` 指定，默认 `e2e/`），加入 `e2e` / `e2e:watch` 脚本并把下载产物目录追加进 `.gitignore`（幂等，手改过的文件在 `force=true` 前保留），`status` 只报告现状与确切的安装命令、不写任何文件；runner 依赖留在你的项目里。
