...
给 DeepSeek Harness（DSH）智能体提供 Obsidian 插件开发能力：脚手架、构建、离线冒烟、部署、沙箱化端到端测试、真机观察与验证、校验、版本同步，并配套开发规范 skill。
...
| **L3** 用户的 Obsidian | `obsidian_plugin_vault` / `obsidian_plugin_reload` / `obsidian_plugin_inspect` / `eval` 经 CLI 作用于**你自己的**运行中 App，仅用于验证你的真实环境 | 会切换你的窗口、抢焦点 |
