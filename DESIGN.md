...
2. **护栏（tool bundle）**——本仓库 `dsh-obsidian-plugin`，用 typed schema 暴露 3 个工具，把「脚手架、校验、版本同步」这些容易出错的确定性操作封装起来。
...
└───────▲──────────────────────────▲─────────┘
        │ 工具(bundle)             │ skill 发现
┌───────┴──────────────┐  ┌────────┴───────────────────┐
│ dsh-obsidian-plugin    │  │ obsidian skill（submodule） │
...
├── cordis.patch.yml      # insert dsh-obsidian-plugin -> dsh-obsidian-plugin
...
- **bundle manifest**：`package.json` 声明 `dsh.bundle.patch: ./cordis.patch.yml`；patch 用 `insert` 挂载 `id: dsh-obsidian-plugin, name: 'dsh-obsidian-plugin'`。缺失该声明时 loader 报 `declares no dsh.bundle`。
...
1. `dsh --profile web --dump-config` —— 组合层出现 `# == dsh-obsidian-plugin` 与 `id: dsh-obsidian-plugin`。
