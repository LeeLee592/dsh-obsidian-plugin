...
└───────▲──────────────────────────▲─────────┘
        │ 工具(bundle)             │ skill 发现
┌───────┴──────────────────────┐  ┌────────┴───────────────────┐
│ @leelee/dsh-obsidian-plugin │  │ obsidian skill（submodule） │
...
- **bundle manifest**：`package.json` 声明 `dsh.bundle.patch: ./cordis.patch.yml`；patch 用 `insert` 挂载 `id: obsidian-plugin, name: '@leelee/dsh-obsidian-plugin'`。缺失该声明时 loader 报 `declares no dsh.bundle`。
