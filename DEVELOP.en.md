...
- **Skill distribution**: to make a skill available under any project root, ship it under `assets/skills/` and register it as a runtime skill in `apply()` via `ctx.get("skills")?.register({...})` (body read from `new URL('../assets/skills/<name>/SKILL.md', import.meta.url)`); don't put it under `.agents/skills/` (that only serves agents whose project root IS this repo).
